...
// Docling routes
try {
  const doclingRoutes = require('./routes/doclingRoutes');
  app.use('/api/docling', doclingRoutes);
  console.log('Successfully imported Docling routes');
} catch (error) {
  console.warn('Error importing Docling routes:', error.message);
  
  // Create mock Docling routes
  app.get('/api/docling/status', (req, res) => {
    res.status(200).json({
      success: true,
      doclingConfigured: false,
      message: 'Docling routes not available',
      error: 'The Docling routes module could not be loaded'
    });
  });
  
  app.post('/api/docling/process/:id', (req, res) => {
    res.status(500).json({
      success: false,
      error: 'Docling routes not available',
      message: 'The Docling routes module could not be loaded'
    });
  });
}
