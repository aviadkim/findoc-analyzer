/**
 * FinDoc Analyzer UI Components - Enhanced Version
 * Version 2.1 with improved initialization patterns and validation
 */

(function() {
  // Define each component and its methods
  const Components = {
    ProcessButton: {
      initialize: function() {
        console.log('ProcessButton.initialize: Initializing process button component...');

        // Add process button to all pages, not just upload
      ...
      addProcessButtonToUploadForm: function() {
        ...
        if (document.getElementById('process-document-btn')) {
          console.log('Process button already exists, skipping creation');
          return;
        }
        
        // Find the form actions div or any container
        let formActions = document.querySelector('.form-actions');
        
        // If form actions not found, try finding other containers or create one
        if (!formActions) {
          formActions = document.querySelector('form');
          ...
          if (!formActions) {
            ...
            if (mainContent) {
              formActions = mainContent;
            } else {
              // Create a container if none found
              formActions = document.createElement('div');
              formActions.className = 'form-actions';
              formActions.style.margin = '20px 0';
              formActions.style.padding = '15px';
        ...
        if (formActions) {
          ...
          // Add process button after upload button or at the end
          const uploadButton = formActions.querySelector('button.btn-primary') || 
                              formActions.querySelector('button') || 
                              formActions.querySelector('input[type="submit"]');
          ...
          console.error('No suitable container found for process button!');
        }
      },
      ...
      handleProcessButtonClick: function(e) {
        ...
        if (!progressContainer) {
          ...
          // Add progress container to form or body
    ...
    ChatInterface: {
      initialize: function() {
        console.log('ChatInterface.initialize: Initializing chat interface component...');
        this.addChatButton();
      },
      ...
      handleChatButtonClick: function() {
        ...
        if (!chatContainer) {
          ...
          // Add event listeners for chat using bind to maintain context
    ...
    LoginComponents: {
      initialize: function() {
        console.log('LoginComponents.initialize: Initializing login components...');
      ...
      addLoginForm: function() {
        ...
        if (authFormContainer) {
          ...
          if (existingForm) {
            ...
            console.log('Existing login form tagged successfully!');
          } else {
            // Create login form
      ...
      addGoogleLoginButton: function() {
        ...
        if (authDivider) {
          ...
          googleButton.addEventListener('click', this.handleGoogleLoginButtonClick.bind(this));
    ...
    AgentCards: {
      initialize: function() {
        console.log('AgentCards.initialize: Initializing agent cards component...');
      ...
      addAgentCards: function() {
        ...
        if (document.querySelector('.agent-card')) {
          console.log('Agent cards already exist, skipping creation');
          return;
        }
    ...
    ValidationSystem: {
      ...
      initialize: function() {
        console.log('ValidationSystem.initialize: Initializing UI validation system...');
        ...
        // Run validation after a short delay to allow other components to initialize
        setTimeout(() => {
  ...
  // Define main UIComponents object
  const UIComponents = {
    // Store version
    version: '2.1.0',
    ...
    initializeAll: function() {
      console.log(`UIComponents.initializeAll: Initializing all UI components (version ${this.version})...`);
    ...
    initializeComponent: function(componentName) {
    ...
    // Process button initialization - exposed for external calling
    initializeProcessButton: function() {
      console.log('process button init: External call to initialize process button');
      if (this.components.ProcessButton) {
        this.components.ProcessButton.initialize();
        return true;
      }
      return false;
    },
    
    // Chat interface initialization - exposed for external calling
    initializeChatInterface: function() {
      console.log('chat interface init: External call to initialize chat interface');
      if (this.components.ChatInterface) {
        this.components.ChatInterface.initialize();
        return true;
      }
      return false;
    }
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    UIComponents.initializeAll();
  });
  
  // Also try immediate initialization for pages that might be already loaded
  if (document.readyState === 'complete' || document.readyState === 'interactive') {
    console.log('Document already loaded, initializing UI components immediately');
    UIComponents.initializeAll();
  }
  ...
  setTimeout(function() {
    if (!document.getElementById('show-chat-btn')) {
      console.log('Forcing early chat initialization - chat button not found');
      if (UIComponents.components.ChatInterface) {
        UIComponents.components.ChatInterface.initialize();
      }
    }
    
    if (!document.getElementById('process-document-btn')) {
      console.log('Forcing early process button initialization - button not found');
      if (UIComponents.components.ProcessButton) {
        UIComponents.components.ProcessButton.initialize();
      }
    }
  }, 500);
  ...
  // Expose global initialization functions
  window.initProcessButton = UIComponents.initializeProcessButton.bind(UIComponents);
  window.initChatInterface = UIComponents.initializeChatInterface.bind(UIComponents);
})();