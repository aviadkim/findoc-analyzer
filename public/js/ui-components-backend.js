/**
 * FinDoc Analyzer UI Components with Backend Connectivity
 */

// Initialize when DOM is loaded
document.addEventListener('DOMContentLoaded', function() {
  console.log('UI Components with Backend initializing...');
  
  // Add process button to upload form if on upload page
  if (window.location.pathname.includes('/upload')) {
    addProcessButtonWithBackend();
  }
  
  // Add chat button to all pages
  addChatButtonWithBackend();
  
  // Add agent cards if on test page
  if (window.location.pathname.includes('/test')) {
    addAgentCardsWithBackend();
  }
  
  console.log('UI Components with Backend initialized');
});

/**
 * Add process button to upload form with backend connectivity
 */
function addProcessButtonWithBackend() {
  ...
  if (formActions) {
    ...
    console.log('Process button with backend added successfully!');
  } else {
...
/**
 * Add chat button with backend connectivity
 */
function addChatButtonWithBackend() {
  ...
  chatButton.addEventListener('click', function() {
    ...
    if (!chatContainer) {
      ...
      sendButton.addEventListener('click', sendChatMessageWithBackend);
      
      chatInput.addEventListener('keypress', function(e) {
        if (e.key === 'Enter') {
          sendChatMessageWithBackend();
        }
      });
  ...
  console.log('Chat button with backend added successfully!');
}

/**
 * Send chat message with backend connectivity
 */
function sendChatMessageWithBackend() {
...
/**
 * Add agent cards with backend connectivity
 */
function addAgentCardsWithBackend() {
  ...
  xhr.addEventListener('load', function() {
    if (xhr.status === 200) {
      ...
      try {
        ...
        console.log('Agent cards with backend added successfully!');
      } catch (error) {
        console.error('Error parsing response:', error);
