/**
 * Bloomberg Agent API Test
 * 
 * This script tests the Bloomberg Agent API endpoints.
 */
...
const RESULTS_DIR = path.join(__dirname, 'test-bloomberg-api-results');
...
<head>
  ...
  <title>Bloomberg Agent API Test Report</title>
  <style>
    body {
...
<body>
  <h1>Bloomberg Agent API Test Report</h1>
...
async function runTests() {
  try {
    ...
    // Test 7: Answer comparison question
    await runTest('Answer comparison question', async () => {
      const question = 'Compare Apple and Microsoft stock performance';
      ...
      // Save answer to file
      fs.writeFileSync(path.join(RESULTS_DIR, 'comparison-question-answer.json'), JSON.stringify(result, null, 2));
    });
    
    // Test 8: Answer analysis question
    await runTest('Answer analysis question', async () => {
      const question = 'Analyze NVDA stock';
      ...
      // Save answer to file
      fs.writeFileSync(path.join(RESULTS_DIR, 'analysis-question-answer.json'), JSON.stringify(result, null, 2));
    });
  } catch (error) {
