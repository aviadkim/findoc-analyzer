...
class PdfProcessor {
  ...
  async analyzeText(text) {
    try {
      if (!this.openai) {
        console.warn('OpenAI API key not provided, using mock data');
  ...
  async answerQuestion(documentText, question) {
    try {
      if (!this.openai) {
        console.warn('OpenAI API key not provided, using mock answers');
   ...
   */
  async generateTable(documentText, prompt) {
    try {
      if (!this.openai) {
        console.warn('OpenAI API key not provided, using mock tables');
        return this.getMockTable(prompt);
      }
      
      // Call OpenAI API to generate a table
      const completion = await this.openai.chat.completions.create({
        model: "gpt-4",
        messages: [
          {
  ...
  getMockAnalysis(text) {
    ...
    const title = lines[0] || "Messos Group Annual Financial Report";
  ...
  /**
   * Get mock table for testing
   * @param {string} prompt - Prompt
   * @returns {Array<object>} - Mock table data
   */
  getMockTable(prompt) {
    // Simple pattern matching for demo purposes
