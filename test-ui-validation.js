...
async function validatePage(page, url) {
  ...
  for (const element of elementsToValidate) {
    ...
    const isVisible = found ? await found.isVisible() : false;
...
async function main() {
  ...
  fs.writeFileSync('ui-validation-results.json', JSON.stringify(allResults, null, 2));
  console.log('Results saved to ui-validation-results.json');
  ...
  fs.writeFileSync('ui-validation-summary.json', JSON.stringify(summary, null, 2));
  console.log('Summary saved to ui-validation-summary.json');
