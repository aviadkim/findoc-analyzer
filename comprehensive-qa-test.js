 ...
 * 
 * This script performs hundreds of tests on the deployed application to identify
...
// Main test function
const runComprehensiveTests = async (url = config.baseUrl) => {
  console.log(`\n🚀 Starting comprehensive QA tests for FinDoc Analyzer at ${url}`);
  ...
  try {
    ...
    await runTest('Basic Navigation', async () => {
      ...
      for (const link of navLinks) {
        ...
        await page.waitForLoadState('networkidle');
        await takeScreenshot(page, `nav-${link.text.toLowerCase()}`);
        
        // Check if URL changed correctly
        const currentUrl = page.url();
        if (!currentUrl.includes(link.url)) {
          await logBug(page, `Navigation to "${link.text}" failed. Expected URL to include: ${link.url}, Actual: ${currentUrl}`, 'high', `nav-failed-${link.text}`);
        }
      }
    });
    ...
    await runTest('Document Processing', async () => {
      ...
      if (!processButton) {
        ...
        await page.waitForTimeout(2000);
        await takeScreenshot(page, 'after-process-click');
    ...
    await runTest('Document Chat', async () => {
      ...
      if (!chatContainer) {
        ...
        if (!documentSelect) {
          await logBug(page, 'Document selector not found', 'high', 'missing-document-selector');
        } else {
          // Select a document
          ...
          if (!chatInput) {
            ...
            if (isDisabled) {
              await logBug(page, 'Document chat input is disabled after selecting a document', 'high', 'chat-input-disabled');
            } else {
              // Type a question
              await chatInput.fill('What is the ISIN code for Apple?');
              await takeScreenshot(page, 'question-typed');
              ...
              if (!sendButton) {
                ...
                if (isSendDisabled) {
                  await logBug(page, 'Document chat send button is disabled after selecting a document', 'high', 'chat-send-button-disabled');
                } else {
                  // Click send button
    ...
    await runTest('Analytics', async () => {
      ...
      await page.waitForLoadState('networkidle');
      await takeScreenshot(page, 'analytics');
    ...
    // Test 8: Docling Integration
    await runTest('Docling Integration', async () => {
      await page.goto(`${url}/api/docling/status`);
      await page.waitForLoadState('networkidle');
      
      // Check if Docling status is returned
      const pageContent = await page.content();
      const isDoclingConfigured = pageContent.includes('doclingConfigured');
      
      if (!isDoclingConfigured) {
        await logBug(page, 'Docling status endpoint not working', 'high', 'docling-status-not-working');
      }
      ...
      if (documents.length > 0) {
        ...
        // Check if document detail page has process with Docling button
        const doclingProcessButton = await page.$('.document-detail button:has-text("Process with Docling")');
        if (!doclingProcessButton) {
          await logBug(page, 'Process with Docling button not found on document detail page', 'medium', 'missing-docling-process-button');
        } else {
          // Click process with Docling button
          await doclingProcessButton.click();
          await page.waitForTimeout(5000);
          await takeScreenshot(page, 'docling-processing-result');
          
          // Check if processing started
          const processingIndicator = await page.$('.processing-indicator');
          if (!processingIndicator) {
            await logBug(page, 'No processing indicator after clicking process with Docling button', 'high', 'missing-docling-processing-indicator');
          }
        }
      }
    });
    ...
    await runTest('Mobile Responsiveness', async () => {
      ...
      if (!mobileMenuButton) {
        await logBug(page, 'Mobile menu button not found', 'medium', 'missing-mobile-menu');
      } else {
        ...
        if (navLinks.length === 0) {
          await logBug(page, 'Navigation links not visible in mobile menu', 'medium', 'missing-mobile-nav-links');
        }
      }
      ...
      await page.goto(`${url}/document-chat`);
      await page.waitForLoadState('networkidle');
      await takeScreenshot(page, 'mobile-document-chat');
      
      // Check if chat container is responsive
      const chatContainer = await page.$('#document-chat-container');
      if (chatContainer) {
        const boundingBox = await chatContainer.boundingBox();
        if (boundingBox.width > 375) {
          await logBug(page, 'Document chat container is not responsive', 'medium', 'non-responsive-chat-container');
        }
      }
    ...
    await runTest('Error Handling', async () => {
      // Test 404 page
      await page.goto(`${url}/non-existent-page`);
      await page.waitForLoadState('networkidle');
      ...
      if (!is404Page) {
        await logBug(page, '404 page not displayed for non-existent URL', 'medium', 'missing-404-page');
      }
      
      // Test invalid document ID
      await page.goto(`${url}/documents-new/invalid-id`);
      await page.waitForLoadState('networkidle');
      await takeScreenshot(page, 'invalid-document-id');
      
      // Check if error message is displayed
      const errorMessage = await page.$('.error-message');
      if (!errorMessage) {
        await logBug(page, 'No error message displayed for invalid document ID', 'medium', 'missing-error-message');
      }
    });
    
    // Generate report
    const reportPath = path.join(config.resultsDir, `qa-report-${new Date().toISOString().replace(/:/g, '-')}.json`);
    ...
    const htmlReportPath = path.join(config.resultsDir, `qa-report-${new Date().toISOString().replace(/:/g, '-')}.html`);
    const htmlReport = `
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <div class="bug-list">
    ${testResults.bugs.map(bug => `
      <div class="bug">
        ...
        <img src="${path.relative(config.resultsDir, bug.screenshot)}" alt="Bug Screenshot" class="bug-screenshot">
        <div class="bug-meta">
