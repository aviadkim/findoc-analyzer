/**
 * Enhanced Spreadsheet Test Server
 * 
 * Advanced Express server with real-time processing and visualization
 * for the enhanced spreadsheet processing functionality
 */
...
const browserMcpProcessor = require('./services/browser-mcp-processor');
...
// Routes
app.get('/', (req, res) => {
  const html = fs.readFileSync(path.join(__dirname, 'spreadsheet-test-ui.html'), 'utf8');
  res.send(html);
});
...
app.post('/api/process', upload.single('file'), async (req, res) => {
  try {
    ...
    if (processingType === 'browser-mcp') {
      // Process with browser-based MCP
      result = await browserMcpProcessor.processSpreadsheetWithBrowser(req.file.path, {
        generateVisualizations: true
      });
      
      // Save visualizations if they exist
      if (result.visualizations && result.visualizations.length > 0) {
        result.visualizations = await saveVisualizations(result.visualizations);
      }
    } else {
      // Process with standard MCP
      result = await mcpDocumentProcessor.processDocument(req.file.path);
    }
...
app.get('/api/process-sample', async (req, res) => {
  try {
    const processingType = req.query.processingType || 'standard';
    const samplePath = path.join(__dirname, 'test-portfolio.xlsx');
    
    // Create sample file if it doesn't exist
    if (!fs.existsSync(samplePath)) {
      createSampleExcelFile(samplePath);
    }
    ...
    if (processingType === 'browser-mcp') {
      // Process with browser-based MCP
      result = await browserMcpProcessor.processSpreadsheetWithBrowser(samplePath, {
        generateVisualizations: true
      });
      ...
      // Process with standard MCP
      result = await mcpDocumentProcessor.processDocument(samplePath);
    }
...
async function saveVisualizations(visualizations) {
  ...
  for (let i = 0; i < visualizations.length; i++) {
    ...
    if (viz.dataUrl) {
      const matches = viz.dataUrl.match(/^data:([A-Za-z-+/]+);base64,(.+)$/);
      
      if (matches && matches.length === 3) {
        const type = matches[1];
        const data = Buffer.from(matches[2], 'base64');
        const filename = `visualization-${Date.now()}-${i}.png`;
        const filePath = path.join(visualizationsDir, filename);
        
        fs.writeFileSync(filePath, data);
        
        // Replace data URL with file path
        viz.url = `/visualizations/${filename}`;
        delete viz.dataUrl;
      }
    }
    
    savedVisualizations.push(viz);
  }
  
  return savedVisualizations;
}

// Helper function to create a sample Excel file
function createSampleExcelFile(filePath) {
  console.log(`Creating sample Excel file: ${filePath}`);
...
// Create a beautiful HTML UI
fs.writeFileSync(
  path.join(__dirname, 'spreadsheet-test-ui.html'),
  `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
// Start server
app.listen(port, () => {
  console.log(`Enhanced spreadsheet test server running at http://localhost:${port}`);
});