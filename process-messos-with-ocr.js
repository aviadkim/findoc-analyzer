/**
 * Script to process the messos.pdf file with OCR
 */
...
async function processMessosPdfWithOcr() {
  try {
    console.log(`Processing messos.pdf with OCR...`);
    
    // Process the PDF with OCR
    console.log('Processing PDF with OCR...');
    const pdfData = await processPdf(pdfFile, { useOcr: true });
    ...
    const outputFile = path.join(__dirname, 'messos-extracted-with-ocr.json');
    ...
    console.error('Error processing messos.pdf with OCR:', error);
  }
}

// Run the function
processMessosPdfWithOcr().then(() => {
  console.log('Done!');
}).catch(error => {
