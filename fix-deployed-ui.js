...
// Function to inject UI components into a webpage
async function injectUIComponents(page, url) {
  console.log(`Injecting UI components into ${url}`);
  
  // Navigate to the page
  await page.goto(url, { waitUntil: 'networkidle2' });
  
  // Take a screenshot before injection
  const beforeScreenshotPath = `before-${url.split('/').pop() || 'home'}.png`;
  await page.screenshot({ path: beforeScreenshotPath, fullPage: true });
  console.log(`Screenshot before injection saved to ${beforeScreenshotPath}`);
  
  // Inject UI components script
  ...
  const afterScreenshotPath = `after-${url.split('/').pop() || 'home'}.png`;
  await page.screenshot({ path: afterScreenshotPath, fullPage: true });
  console.log(`Screenshot after injection saved to ${afterScreenshotPath}`);
  
  // Validate UI elements
  const validationResults = await validateUIElements(page);
  
  return validationResults;
}

// Function to validate UI elements
...
async function main() {
  ...
  // Inject UI components into each URL
  for (const url of urls) {
    try {
      allResults[url] = await injectUIComponents(page, url);
    } catch (error) {
      console.error(`Error injecting UI components into ${url}:`, error);
      allResults[url] = { error: error.message };
    }
  }
  
  // Save results to file
  fs.writeFileSync('ui-injection-results.json', JSON.stringify(allResults, null, 2));
  console.log('Results saved to ui-injection-results.json');
  ...
  for (const url in allResults) {
    if (Array.isArray(allResults[url])) {
  ...
  fs.writeFileSync('ui-injection-summary.json', JSON.stringify(summary, null, 2));
  console.log('Summary saved to ui-injection-summary.json');
