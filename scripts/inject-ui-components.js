 ...
 * This script injects UI components into the Docker container
 */
...
const config = {
  ...
  uiFixScript: path.join(__dirname, '../public/js/ui-fix.js'),
  dockerHtmlDir: '/app/public',
...
// Create UI fix script if it doesn't exist
if (!fs.existsSync(config.uiFixScript)) {
  // Create the directory if it doesn't exist
  const uiFixScriptDir = path.dirname(config.uiFixScript);
  if (!fs.existsSync(uiFixScriptDir)) {
    fs.mkdirSync(uiFixScriptDir, { recursive: true });
  }

  // Create the UI fix script
  fs.writeFileSync(config.uiFixScript, `/**
...
function fixDashboardPage() {
  ...
  if (mainContent && !document.querySelector('.recent-documents')) {
    ...
    // Create quick upload section
    const quickUpload = document.createElement('div');
    quickUpload.className = 'quick-upload';
    ...
    mainContent.appendChild(quickUpload);
    console.log('Added quick upload section');
...
function sendChatMessage() {
  ...
  console.log(`Created UI fix script at ${config.uiFixScript}`);
}

// Copy UI fix script to Docker container
function copyUiFixScriptToContainer() {
  console.log('Copying UI fix script to Docker container...');

  try {
    // Create the directory in the container if it doesn't exist
    execSync(`docker exec ${config.containerName} mkdir -p ${path.dirname(config.dockerHtmlDir + '/js/ui-fix.js')}`);

    // Copy the UI fix script to the container
    execSync(`docker cp ${config.uiFixScript} ${config.containerName}:${config.dockerHtmlDir}/js/ui-fix.js`);

    console.log('UI fix script copied to Docker container');
    return true;
  } catch (error) {
    console.error(`Error copying UI fix script to Docker container: ${error.message}`);
    return false;
  }
}

// Add UI fix script to HTML files in Docker container
function addUiFixScriptToHtmlFiles() {
  console.log('Adding UI fix script to HTML files in Docker container...');

  try {
    // Get list of HTML files in the container
    const htmlFiles = execSync(`docker exec ${config.containerName} find ${config.dockerHtmlDir} -name "*.html"`)
      .toString()
      .trim()
      .split('\n')
      .filter(file => file); // Filter out empty lines

    console.log(`Found ${htmlFiles.length} HTML files in Docker container`);

    // Create a script to modify all HTML files
    const modifyScript = `
    for file in ${config.dockerHtmlDir}/*.html; do
      if ! grep -q "ui-fix.js" "$file"; then
        sed -i 's|</body>|  <script src="/js/ui-fix.js"></script>\\n</body>|' "$file"
        echo "Added UI fix script to $file"
      else
        echo "UI fix script already included in $file"
      fi
    done
    `;

    // Write the script to a temporary file
    const tempScriptPath = path.join(__dirname, 'temp-modify-script.sh');
    fs.writeFileSync(tempScriptPath, modifyScript);

    // Copy the script to the container
    execSync(`docker cp ${tempScriptPath} ${config.containerName}:/tmp/modify-script.sh`);

    // Make the script executable and run it
    execSync(`docker exec ${config.containerName} chmod +x /tmp/modify-script.sh`);
    execSync(`docker exec ${config.containerName} /bin/sh /tmp/modify-script.sh`);

    // Clean up
    fs.unlinkSync(tempScriptPath);

    console.log('UI fix script added to HTML files in Docker container');
    return true;
  } catch (error) {
    console.error(`Error adding UI fix script to HTML files in Docker container: ${error.message}`);
    return false;
  }
}
...
// Main function
function main() {
  console.log('Injecting UI components into Docker container...');

  // Copy UI fix script to Docker container
  const scriptCopied = copyUiFixScriptToContainer();

  if (!scriptCopied) {
    console.error('Failed to copy UI fix script to Docker container');
    return;
  }

  // Add UI fix script to HTML files in Docker container
  const scriptAdded = addUiFixScriptToHtmlFiles();

  if (!scriptAdded) {
    console.error('Failed to add UI fix script to HTML files in Docker container');
    return;
  }

  // Restart the Docker container
  const containerRestarted = restartContainer();

  if (!containerRestarted) {
    console.error('Failed to restart Docker container');
    return;
  }

  console.log('UI components injected into Docker container successfully');
}
