...
// Create bookmarklet
const bookmarklet = `javascript:(function(){${uiComponentsScript.replace(/\n/g, ' ').replace(/\s+/g, ' ').replace(/'/g, "\\'").replace(/"/g, '\\"')}})();`;
...
<body>
  ...
  <a href="${bookmarklet}" class="bookmarklet">FinDoc UI Fix</a>
...
// Write HTML file
fs.writeFileSync('ui-bookmarklet.html', htmlContent);
console.log('Bookmarklet created! Open ui-bookmarklet.html in your browser.');

// Create a minified version of the script for direct injection
const minifiedScript = uiComponentsScript.replace(/\n/g, ' ').replace(/\s+/g, ' ');
fs.writeFileSync('ui-components-minified.js', minifiedScript);
console.log('Minified script created! Use ui-components-minified.js for direct injection.');

// Create a script tag version for adding to HTML files
const scriptTagVersion = `<script>\n${uiComponentsScript}\n</script>`;
fs.writeFileSync('ui-components-script-tag.html', scriptTagVersion);
console.log('Script tag version created! Use ui-components-script-tag.html to add to HTML files.');

// Create a README file
const readmeContent = `# FinDoc Analyzer UI Fix

This package contains scripts to fix the missing UI elements in the FinDoc Analyzer application.

## Files

- **ui-bookmarklet.html**: HTML file with a bookmarklet that can be dragged to your bookmarks bar and clicked to add the missing UI elements.
- **ui-components-minified.js**: Minified version of the UI components script for direct injection.
- **ui-components-script-tag.html**: Script tag version of the UI components script for adding to HTML files.
...
You can also inject the UI components directly into the application by adding the script tag to the HTML files:
...
- Google Login Button
- Show Chat Button
- Agent Cards (on test page)
`;

fs.writeFileSync('UI-FIX-README.md', readmeContent);
console.log('README created! See UI-FIX-README.md for instructions.');

console.log('\nAll files created successfully!');
console.log('Open ui-bookmarklet.html in your browser to use the bookmarklet.');
