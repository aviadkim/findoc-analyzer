...
class DocumentRepository {
  ...
  private async getAllDocumentsFromApi(): Promise<Document[]> {
    try {
  ...
  private async getDocumentByIdFromApi(id: string): Promise<Document | null> {
    try {
  ...
  private async uploadDocumentToApi(file: File, options?: DocumentUploadOptions): Promise<Document> {
    try {
      ...
      const response = await fetch('http://localhost:24125/api/documents/upload', {
        method: 'POST',
        body: formData,
      });
