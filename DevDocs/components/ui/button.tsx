  ...
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        ...
        subtle: "bg-primary-100 text-primary-800 hover:bg-primary-200 dark:bg-primary-900/30 dark:text-primary-300 dark:hover:bg-primary-900/50",
        accent: "bg-accent text-accent-foreground hover:bg-accent/90",
        soft: "bg-background border border-border/50 text-foreground hover:border-border hover:bg-accent/50 hover:text-accent-foreground",
      },
      size: {
    ...
    compoundVariants: [
      {
        ...
        className: "active:scale-[0.98] active:shadow-none",
      },
    ],
  }
)
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({
    ...
    return (
      <Comp
        className={cn(buttonVariants({ variant, size, emphasis, rounded, animation, className }))}
        ref={ref}
