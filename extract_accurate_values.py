"""
Script to extract accurate values from the details field of each security.
"""
import os
import sys
import json
...
def extract_accurate_values(securities: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Extract accurate values from the details field of each security.
    
    Args:
        securities: List of securities
        
    Returns:
        List of securities with accurate values
    """
    enhanced_securities = []
    ...
    for security in securities:
        ...
        if 'details' in security:
            for detail in security['details']:
                # Look for patterns like "USD 200'000 ... 199'172 1.02%"
                ...
                value_match = re.search(r'USD\s+(\d+\'?\d*\'?\d*)\s+.*?(\d+\'?\d*\'?\d*)\s+\d+\.\d+%\s+-?\d+\.\d+%\s+(\d+\'?\d*\'?\d*)', detail)
                if value_match:
                    enhanced['nominal_value'] = value_match.group(1).replace("'", "")
                    enhanced['actual_value'] = value_match.group(3).replace("'", "")
                    break
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python extract_accurate_values.py <json_file>")
        sys.exit(1)
    ...
    if 'securities' in data:
        data['securities'] = extract_accurate_values(data['securities'])
    
    # Save the enhanced JSON file
    output_file = f"{os.path.splitext(json_file)[0]}_accurate.json"
