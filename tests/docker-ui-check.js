/**
 * Docker UI Components Check
 * This script checks for the presence of UI components in the Docker container
 */
...
const resultsDir = path.join(__dirname, 'docker-results');
...
<head>
  ...
  <title>Docker UI Test Results</title>
  <style>
    body {
...
<body>
  <h1>FinDoc Analyzer Docker UI Test Results</h1>
  ...
  ` : `
    <p>All UI components are present. You can now deploy the application to Google Cloud Run.</p>
  `}
</body>
</html>
  `;
  ...
  // Save HTML report
  fs.writeFileSync(path.join(resultsDir, 'docker-ui-test-report.html'), reportHTML);
  console.log(`Report saved to ${path.join(resultsDir, 'docker-ui-test-report.html')}`);
  
  // Save JSON results
  fs.writeFileSync(path.join(resultsDir, 'docker-ui-test-results.json'), JSON.stringify(results, null, 2));
  console.log(`Results saved to ${path.join(resultsDir, 'docker-ui-test-results.json')}`);
}
