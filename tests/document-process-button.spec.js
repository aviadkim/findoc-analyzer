...
/**
 * Document Process Button Test
 * 
 * This test verifies that the document process button works correctly.
 */

test.describe('Document Process Button Test', () => {
  test('Process button on document detail page', async ({ page }) => {
    ...
    await page.screenshot({ path: 'test-results/06-document-details.png' });
    ...
    if (await processButton.isVisible()) {
      ...
      await page.screenshot({ path: 'test-results/07-processing-started.png' });
      
      // Wait for a while to let processing happen
      await page.waitForTimeout(5000);
      
      // Take a screenshot after waiting
      await page.screenshot({ path: 'test-results/08-after-waiting.png' });
      ...
      // Take a screenshot of the processing status
      await page.screenshot({ path: 'test-results/07-reprocessing-started.png' });
      ...
      if (await processBtn.isVisible()) {
        ...
        // Take a screenshot after waiting
        await page.screenshot({ path: 'test-results/07-processing-from-list.png' });
        ...
        await page.screenshot({ path: 'test-results/07-no-process-button.png' });
      }
    }
  });
});
